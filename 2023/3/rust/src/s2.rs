...
pub fn run(  )
{
    ...
    let mut symbol_list : HashMap<String,Symbol> = HashMap::new();
    ...
    for kv_line in input_text.iter().enumerate()
    {
        ...
        for kv_letter in kv_line.1.chars().enumerate()
        {
           ...
           if kv_letter.1 == '*'
           {
               let parsed_symbol = Symbol{
                   x :kv_letter.0 as i32 ,
                   y :kv_line.0 as i32,
                   len : 1,
               };
               
               symbol_list.insert( format!("{}:{}",kv_line.0,kv_letter.0), parsed_symbol );
           }
        }
    }
    ...
    if in_number
    {
        println!("FOUND END OF GRID NUMBER");
        let num_len: i32 = tmp_num.len() as i32; 
        in_number = false;
        let parsed_num = Number{
            num :tmp_num.parse().unwrap(),
    ...
    let mut gear_close_list: HashMap<String, Vec<i32>  > =  HashMap::new();
        ...
        'y_loop: for y in num_y-1..=num_y+1
        {
            for x in start_x-1..=end_x
            {
                ...
                if symbol_list.contains_key(&idx)
                {
                    ...
                    if gear_close_list.contains_key(&idx) == false
                    {
                        gear_close_list.insert(
                            idx.clone(),
                            Vec::new()
                    );
                    }
                    if let Some(x) =gear_close_list.get_mut(&idx){
                       x.push(num_tup.1.num); 
                    };
                }
            }
        }
    ...
    for kv_gear_stats in gear_close_list
    {

        if  kv_gear_stats.1.len() == 2
        {
            println!("-------");
            println!("idx {}",kv_gear_stats.0);

            for numbers in &kv_gear_stats.1
            {
                println!("  cnum: {}",numbers);
            }

            sum+=(kv_gear_stats.1.get(0).unwrap()*kv_gear_stats.1.get(1).unwrap());
        }
    }
